...
WebApp.connectHandlers.use(function (req, res, next) {
  ...
  if (/\?.*_escaped_fragment_=/.test(req.url) ||
      _.any(Spiderfork.userAgentRegExps, function (re) {
    ...
    child_process.execFile(
      '/bin/bash',
      ['-c',
       ("exec phantomjs --debug=true --ssl-protocol=TLSv1 --ignore-ssl-errors=true --load-images=no /dev/stdin <<'END'\n" +
        phantomScript + "END\n")],
      ...
      function (error, stdout, stderr) {
        if (!error && /<html/i.test(stdout)) {
          ...
          if (error && error.code === 127)
            Meteor._debug("spiderfork: phantomjs not installed. Download and install from http://phantomjs.org/");
          else
            Meteor._debug("spiderfork: phantomjs failed:", error, "\nstderr:", stderr);
